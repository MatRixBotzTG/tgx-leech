...
async def restart_notification():
    ...
    class MockQuery:
        def __init__(self, message):
            ...
            self.from_user = type('MockUser', (), {'id': int(config_dict['OWNER_ID'])})()
            self.data = "resume yes"
        async def answer(self, text=None, show_alert=None, url=None):
            pass
        async def edit_message_text(self, text, parse_mode=None, disable_web_page_preview=None, reply_markup=None):
            pass

    async def send_incompelete_task_message(cid, msg, reply_markup=None):
        try:
            if msg.startswith("⌬ <b><i>Restarted Successfully!</i></b>"):
                sent_msg = await bot.edit_message_text(chat_id=chat_id, message_id=msg_id, text=msg, disable_web_page_preview=True, reply_markup=reply_markup)
                await aioremove(".restartmsg")
                return sent_msg
            else:
                sent_msg = await bot.send_message(chat_id=cid, text=msg, disable_web_page_preview=True, disable_notification=True, reply_markup=reply_markup)
                return sent_msg
        except Exception as e:
            LOGGER.error(e)
            return None
                ...
                for tag, links in data.items():
                    ...
                    for index, link in enumerate(links, start=1):
                        ...
                        if len(msg.encode()) > 4000:
                            if config_dict['RESUME_TASK']:
                                sent_msg = await bot.send_message(chat_id=cid, text=msg + "\n\n<b><i>Auto-Resuming Tasks...</i></b>", disable_web_page_preview=True, disable_notification=True)
                                if hasattr(resume_task, 'start_resume_task'):
                                    await resume_task.start_resume_task(bot, MockQuery(sent_msg))
                            else:
                                await send_incompelete_task_message(cid, msg, reply_markup)
                            msg = ''
                if msg:
                    if config_dict['RESUME_TASK']:
                        final_text = msg + "\n\n<b><i>Auto-Resuming Tasks...</i></b>"                       
                        if msg.startswith("⌬ <b><i>Restarted Successfully!</i></b>"):
                            sent_msg = await bot.edit_message_text(chat_id=chat_id, message_id=msg_id, text=final_text, disable_web_page_preview=True)
                            await aioremove(".restartmsg")
                        else:
                            sent_msg = await bot.send_message(chat_id=cid, text=final_text, disable_web_page_preview=True, disable_notification=True)
